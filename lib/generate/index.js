...
function substitutions(answers) {
  ...
  }, tenancySubstitutions(answers.tenancy || []));
}
...
function tenancySubstitutions(levels) {
  ...
  return {
    ...
    __MT_FACTORY_AUTH__: on ? [
      '        // …and only for a ' + first.label.toLowerCase() + ' the caller belongs to.',
      '        auth: tenants.memberGate'
    ].join('\n') : '',
    ...
    __MT_DRAWER_ITEM__: on ? "    { name: 'Switch " + first.label.toLowerCase() + "', icon: SwitchIcon, clickHandler: () => navigate('/select') }," : '',
