...
async function collect(session, argvName) {
  ...
  // ── flows ─────────────────────────────────────────────────────────────
  //
  // Screens one after another, where what someone filled in decides the next
  // screen. The journey is run by Xeplr Workflow — a separate service, not a
  // package — so this only asks where it is. Off, the app has no flows at all.
  var workflow = null;
  var wantsFlows = await session.confirm('Flows — is Xeplr Workflow running for this app?', false);
  if (wantsFlows) {
    var workflowUrl = await session.ask('  Workflow API address?', {
      default: 'http://localhost:19122',
      validate: function (v) { return /^https?:\/\/[^\s]+$/.test(v) ? true : 'An http:// or https:// address.'; }
    });
    // Workflow keeps its access rules (who may design a flow, who may run one)
    // in migrations-auth/. This app's sign-in service has to load them, so it
    // needs to know where they are. Blank is allowed: the README says how.
    var workflowDir = await session.ask('  Workflow backend folder (for its access rules — blank to add later)?', {
      default: '',
      validate: function (v) { return !v || !/\s/.test(v) ? true : 'A path without spaces.'; }
    });
    workflow = { url: workflowUrl.replace(/\/+$/, ''), dir: workflowDir.replace(/\/+$/, '') };

    // Workflow files every flow and every run under a company and a workspace
    // — the two levels it registers, with those headers. An app that sends
    // anything else builds, starts, and then has every flow refused, so the
    // levels that make flows work are set here, and said out loud.
    if (!sameLevels(tenancy, tenancyLevels(WORKFLOW_LEVELS))) {
      tenancy = tenancyLevels(WORKFLOW_LEVELS);
      if (session.say) {
        session.say('  Flows need the same tenancy as Xeplr Workflow — company, then workspace —');
        session.say('  so this app uses those two levels.');
      }
    }
  }
...
/** The tenancy Xeplr Workflow registers: a flows app must send the same headers. */
var WORKFLOW_LEVELS = 'company, workspace';

/** Same levels, in the same order, with the same headers. */
function sameLevels(a, b) {
  return a.length === b.length && a.every(function (l, i) { return l.header === b[i].header; });
}

module.exports = { collect: collect, envPrefix: envPrefix, tenancyLevels: tenancyLevels, sameLevels: sameLevels, WORKFLOW_LEVELS: WORKFLOW_LEVELS, NAME_RE: NAME_RE };
